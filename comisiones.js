// comisiones.js — Motor Financiero SMNYL v4
// Cuadernos 2026: Asesores en Desarrollo + Nuevos Profesionales
// Comisiones: Tabla Enero 2025_V1 + GMM post-circular
...
// ═══════════════════════════════════════════════════════════════════════════
// TASAS DE COMISIÓN — por año de póliza
// índices: [año1, año2, año3, año4-5, año6-10, año11+]
// ═══════════════════════════════════════════════════════════════════════════
const TASAS_VIDA = {
    ...
    'Segubeca':           { default:[0.33,0.10,0.07,0.03,0.03,0] },
};
...
// Planes que NO cuentan puntos independientemente de la prima
...
// NO "vidas" — son PUNTOS según prima anualizada
...
// Ponderación prima meta para bono (Tabla 1 cuadernos 2026)
...
// Training Allowance — Asesor en Desarrollo (Tabla 2 del cuaderno)
...
// Nuevos Profesionales — grupos prima meta semestral (Tabla 2 NP)
const NP_GRUPOS = [
...
// % Bono Inicial Vida según grupo y LIMRA (Tabla 5 NP)
const NP_BONO_PCT = {
...
// GMM Tabla 7 NP — grupos trimestral
const GMM_GRUPOS = [
...
function calcularMotor(cartera, perfil) {
    ...
    // Detalle pólizas mes actual para tabla
    const detallesMes = [];
    // Simulador
    let simItems = [];
    ...
    cartera.forEach(p => {
        ...
        const fpF     = factorPago(p.formaPago);
        const primaRecibo = prima * fpF;
        ...
        if(mesEm===mes && anioEm===anio) {
            ...
            detallesMes.push({
                cliente:p.cliente||'—', plan:p.plan, variante:p.variante||'',
                prima, formaPago:p.formaPago||'Anual', anioP, tasa,
                comRecibo, esRenov, esGMM, puntos
            });
        }
    ...
    const proyeccionAnual = comYTD + (promedioMensual * mesesRestantes);

    return {
        mesConcurso, esDesarrollo, factorD,
        comInicialMes, comRenovMes, puntosMes, primaMetaMes,
        comInicialSem, puntosSem, primaMetaSem,
        primaGMMtrim, polsGMMtrim,
        comMesPasado, polsMesPasado,
        comYTD, comInicialYTD,
        proyeccionAnual, promedioMensual,
        hist6, etiq6, detallesMes, bono,
    };
}
...
export function renderComisiones() {
    return `<div id="fin-root" style="display:flex;flex-direction:column;gap:0;padding-bottom:100px;">
        <div class="skeleton-shimmer" style="height:140px;border-radius:24px;margin:16px;"></div>
        <div class="skeleton-shimmer" style="height:80px;border-radius:20px;margin:0 16px 16px;"></div>
        <div class="skeleton-shimmer" style="height:220px;border-radius:20px;margin:0 16px 16px;"></div>
    </div>`;
}
...
export async function bindComisionesEvents() {
    ...
    try {
        ...
        showToast('Error al cargar el módulo financiero.','danger');
    }
}

// ── Config form ────────────────────────────────────────────────────────────
function renderConfigForm() {
    return `<div class="card" style="border-left:4px solid var(--accent);margin:16px;">
        <h2 style="font-size:18px;margin-bottom:4px;">⚙️ Configurar Perfil Financiero</h2>
        <p style="font-size:13px;color:var(--text-secondary);margin-bottom:16px;">
            Con tu fecha de conexión detecto automáticamente tu cuaderno y calculo tus bonos.
        </p>
        <div style="display:flex;flex-direction:column;gap:12px;">
            <div>
                <label style="font-size:11px;font-weight:600;color:var(--text-secondary);text-transform:uppercase;">Fecha de Conexión</label>
                <input type="date" id="cfg-fec" style="width:100%;margin-top:4px;">
                <p style="font-size:11px;color:var(--text-tertiary);margin-top:4px;">Mes 1-12 → Training Allowance. Mes 13+ → Nuevo Profesional.</p>
            </div>
            <div>
                <label style="font-size:11px;font-weight:600;color:var(--text-secondary);text-transform:uppercase;">LIMRA actual %</label>
                <input type="number" id="cfg-limra" placeholder="Ej. 82.5" step="0.1" min="0" max="100" value="75.5" style="width:100%;margin-top:4px;">
            </div>
            <div>
                <label style="font-size:11px;font-weight:600;color:var(--text-secondary);text-transform:uppercase;">IGC actual %</label>
                <input type="number" id="cfg-igc" placeholder="Ej. 93.0" step="0.1" min="0" max="100" value="91.0" style="width:100%;margin-top:4px;">
            </div>
            <button id="btn-save-cfg" class="btn-primary" style="margin-top:8px;">🚀 Iniciar Motor Financiero</button>
        </div>
    </div>`;
}
...
function bindConfigForm(sb, userId) {
    document.getElementById('btn-save-cfg')?.addEventListener('click', async () => {
        ...
        await sb.from('perfil_asesor').upsert([{user_id:userId,fecha_conexion:f,esquema,limra,igc}]);
...
function buildUI(r, perfil) {
    ...
    const graficaHTML = hist6.map((h,i)=>{
        ...
        return `<div style="flex:1;display:flex;flex-direction:column;align-items:center;gap:2px;">
            ...
            <div style="width:100%;display:flex;flex-direction:column;justify-content:flex-end;height:90px;gap:1px;border-radius:8px;overflow:hidden;background:var(--surface-2);">
                ...
                ${pI>0?`<div style="width:100%;height:${pI}%;background:${esAct?'var(--accent)':'var(--accent)'};opacity:${esAct?1:0.45};min-height:3px;"></div>`:''}
            </div>
    ...
    const tablaFilas = detallesMes.length===0
        ? `<tr><td colspan="6" style="text-align:center;padding:20px;color:var(--text-tertiary);font-size:13px;">Sin movimientos registrados este mes</td></tr>`
        : detallesMes.map(d=>`
            <tr style="border-bottom:1px solid var(--separator);">
                ...
                <td style="padding:10px 4px;font-size:12px;color:var(--text-secondary);">
                    ${d.plan}<br>
                    <span style="font-size:10px;color:var(--text-tertiary);">${d.formaPago} · Año ${d.anioP}</span>
                </td>
                ...
                <td style="padding:10px 4px;font-size:11px;text-align:center;">
                    <span style="background:var(--accent-soft);color:var(--accent);padding:2px 6px;border-radius:8px;font-weight:600;">${(d.tasa*100).toFixed(1)}%</span>
                </td>
        ...
        <div class="card" style="border-left:4px solid var(--warning);margin:0 16px 16px;">
            <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:14px;">
                ...
                <div style="display:flex;flex-direction:column;align-items:flex-end;gap:2px;">
                    <span style="font-size:11px;color:var(--text-secondary);background:var(--surface-2);padding:3px 8px;border-radius:20px;">Mes ${bono.mc} de concurso</span>
            ...
            <div style="margin-bottom:12px;">
                <div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:4px;">
                    ...
                    <span style="font-weight:700;color:var(--text-primary);">${fmt(r.comInicialSem)} <span style="color:var(--text-tertiary);font-weight:400;">/ ${fmt(bono.meta.comAcum)}</span></span>
                </div>
                <div style="background:var(--surface-2);border-radius:6px;height:8px;overflow:hidden;">
                    <div style="background:var(--accent);width:${pctC}%;height:100%;border-radius:6px;transition:width .6s ease;"></div>
                </div>
            </div>
            ...
            <div style="margin-bottom:16px;">
                <div style="display:flex;justify-content:space-between;font-size:12px;margin-bottom:4px;">
                    ...
                    <span style="font-weight:700;color:var(--text-primary);">${fmtN(r.puntosSem)} <span style="color:var(--text-tertiary);font-weight:400;">/ ${bono.meta.ptosAcum}</span></span>
                </div>
                <div style="background:var(--surface-2);border-radius:6px;height:8px;overflow:hidden;">
                    <div style="background:var(--success);width:${pctP}%;height:100%;border-radius:6px;transition:width .6s ease;"></div>
                </div>
            </div>
            ...
            ${bono.cumple
                ...
                : `<div style="background:rgba(255,149,0,.1);border:1px solid var(--warning);border-radius:12px;padding:12px;font-size:13px;color:var(--warning);">
                    ⚠️ Falta <strong>${fmt(bono.fCom)}</strong> en comisiones y <strong>${fmtN(bono.fPtos)} puntos</strong> para calificar.
                   </div>`}
        </div>`;
    } else {
        // Nuevo Profesional
        const grupoTxt=bono.grupo?`Grupo ${bono.grupo}`:'Sin grupo aún';
        const gGMMtxt=bono.grupoGMM?`Grupo ${bono.grupoGMM.g}`:'Sin grupo GMM';
        bonoHTML=`
        <div class="card" style="border-left:4px solid var(--warning);margin:0 16px 16px;">
            <h2 style="font-size:15px;margin-bottom:14px;">🏆 Bonos Nuevo Profesional</h2>
            ...
            <div style="background:var(--surface-2);border-radius:14px;padding:14px;margin-bottom:10px;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                    ...
                    <span class="badge badge-blue">${grupoTxt}</span>
                </div>
                <div style="font-size:12px;color:var(--text-secondary);margin-bottom:8px;">
                    Prima meta sem: <strong>${fmt(r.primaMetaSem)}</strong> · LIMRA: <strong>${bono.limra}%</strong> · Bono: <strong>${(bono.pct*100).toFixed(1)}%</strong>
                </div>
                <div style="font-size:28px;font-weight:800;color:${bono.montoBI>0?'var(--accent)':'var(--text-secondary)'};">${fmt(bono.montoBI)}</div>
                ${!bono.grupo?`<p style="font-size:11px;color:var(--text-secondary);margin:6px 0 0;">Necesitas al menos ${fmt(275000)} en prima meta semestral para calificar al grupo 16.</p>`:''}
            </div>

            <div style="background:var(--surface-2);border-radius:14px;padding:14px;">
                <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
                    <span style="font-size:13px;font-weight:600;color:var(--text-primary);">Bono GMM Trimestral</span>
                    <span class="badge ${bono.grupoGMM?'badge-green':'badge-orange'}">${gGMMtxt}</span>
                </div>
                <div style="font-size:12px;color:var(--text-secondary);margin-bottom:8px;">
                    Prima GMM trimestre: <strong>${fmt(r.primaGMMtrim)}</strong> · Pólizas GMM: <strong>${r.polsGMMtrim}</strong>
                </div>
                <div style="font-size:28px;font-weight:800;color:${bono.montoGMM>0?'var(--success)':'var(--text-secondary)'};">${fmt(bono.montoGMM)}</div>
            </div>

            <div style="background:var(--surface-2);border-radius:14px;padding:14px;margin-top:10px;text-align:center;">
                <span style="font-size:11px;color:var(--text-secondary);font-weight:600;text-transform:uppercase;">Total bonos proyectados</span>
    ...
    <div class="card" style="border-left:4px solid #5856D6;margin:0 16px 16px;">
        ...
        <p style="font-size:12px;color:var(--text-secondary);margin-bottom:12px;">¿Cuánto ganaría si cierro más pólizas este mes?</p>
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;margin-bottom:12px;">
            ...
            <div>
                ...
                <input type="number" id="sim-prima" placeholder="$25,000" style="width:100%;margin-top:4px;">
            </div>
            <div>
                <label style="font-size:11px;color:var(--text-secondary);">Cantidad de pólizas</label>
        ...
        <div id="sim-resultado" style="background:var(--surface-2);border-radius:12px;padding:14px;font-size:13px;color:var(--text-primary);min-height:50px;display:none;"></div>
    </div>`;
    ...
    <div class="card" style="margin:0 16px 16px;">
        ...
        <p style="font-size:11px;color:var(--text-tertiary);margin-top:8px;">Basada en el promedio de los últimos ${hist6.filter(h=>h.ini+h.ren>0).length} meses con actividad.</p>
    </div>`;
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px;margin:0 16px 16px;">
        ...
        <div class="card" style="margin:0;border-left:4px solid var(--separator);">
            <span style="font-size:11px;color:var(--text-secondary);font-weight:600;text-transform:uppercase;">Comisiones YTD</span><br>
    ...
    <!-- ④ Tabla detalle pólizas del mes (iOS card) -->
    <div class="card" style="margin:0 16px 16px;">
        <h2 style="font-size:15px;margin-bottom:4px;">📋 Detalle de Comisiones — ${mesNom}</h2>
        <p style="font-size:12px;color:var(--text-secondary);margin-bottom:12px;">Pólizas que generaron comisión este mes.</p>
            ...
            <table style="width:100%;border-collapse:collapse;min-width:480px;">
                ...
                <tfoot>
                    <tr style="border-top:2px solid var(--separator);">
                        <td colspan="4" style="padding:10px 4px 0;font-size:13px;font-weight:700;color:var(--text-primary);">Total</td>
                        <td style="padding:10px 0 0;text-align:right;font-size:15px;font-weight:800;color:var(--accent);">${fmt(r.comInicialMes+r.comRenovMes)}</td>
                        <td style="padding:10px 0 0 4px;text-align:center;font-size:13px;font-weight:700;color:var(--text-primary);">${fmtN(r.puntosMes)}</td>
                    </tr>
                </tfoot>`:''}
            </table>
        </div>
    ...
    <div class="card" style="margin:0 16px 16px;">
        ...
        <div style="display:flex;justify-content:center;gap:20px;margin-top:12px;">
            <div style="display:flex;align-items:center;gap:6px;font-size:11px;color:var(--text-secondary);">
                <div style="width:10px;height:10px;background:var(--accent);border-radius:3px;"></div>Iniciales
            </div>
            <div style="display:flex;align-items:center;gap:6px;font-size:11px;color:var(--text-secondary);">
                <div style="width:10px;height:10px;background:var(--success);border-radius:3px;"></div>Renovación
            </div>
        </div>
    </div>
    ...
    <div class="card" style="border-left:4px solid var(--accent);margin:0 16px 16px;">
        ...
        <div id="out-tips" style="font-size:13px;color:var(--text-primary);line-height:1.7;min-height:60px;white-space:pre-wrap;">
            Presiona "Generar" para recibir estrategias personalizadas con tus números reales.
        </div>
    </div>
    ...
    <div style="margin:0 16px 16px;text-align:center;">
        <button id="btn-dev-reset" class="btn-secondary" style="opacity:.35;font-size:11px;">⚙️ Modo Dev: Resetear Perfil Financiero</button>
    </div>`;
}
...
function bindUIEvents(r, perfil, sb, userId) {
    ...
    document.getElementById('btn-simular')?.addEventListener('click', () => {
        ...
        if(!prima||prima<=0) return showToast('Ingresa una prima válida.','danger');
        ...
        const comTotal = comPol * cant;
        const puntos   = puntosPoliza(plan, prima, esGMM) * cant;

        let bonoExtra = '';
        if(r.esDesarrollo && bono.tipo==='training'){
            const nuevaCom = r.comInicialSem + comTotal;
            const nuevosPtos = r.puntosSem + puntos;
            const fCom = Math.max(0, bono.meta.comAcum - nuevaCom);
            const fPtos= Math.max(0, bono.meta.ptosAcum - nuevosPtos);
            const cumple = fCom<=0 && fPtos<=0;
            bonoExtra = cumple
                ? `<div style="margin-top:8px;color:var(--success);font-weight:600;">✅ Con estas pólizas calificarías al bono del mes ${bono.mc}.</div>`
                : `<div style="margin-top:8px;color:var(--warning);">Aún faltarían ${fmt(fCom)} y ${fmtN(fPtos)} puntos.</div>`;
        }
        ...
        res.innerHTML = `
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:8px;">
                <div style="text-align:center;"><span style="font-size:10px;color:var(--text-secondary);">Comisión / póliza</span><br><strong style="color:var(--accent);">${fmt(comPol)}</strong></div>
                <div style="text-align:center;"><span style="font-size:10px;color:var(--text-secondary);">Comisión total</span><br><strong style="color:var(--accent);font-size:16px;">${fmt(comTotal)}</strong></div>
            ...
            <div style="font-size:12px;color:var(--text-secondary);">Tasa aplicada: ${(tasa*100).toFixed(1)}% ${r.factorD<1?'(factor 90% Desarrollo)':''}</div>
            ${bonoExtra}`;
    });
    ...
    document.getElementById('btn-gen-tips')?.addEventListener('click', () => {
        ...
        let ctx = '';
        if(bono.tipo==='training'){
            ctx=`Cuaderno: Asesor en Desarrollo, Mes ${bono.mc} de concurso
Factor comisión: ${r.factorD===0.9?'90% (primeros 12 meses)':'100%'}
Comisión acumulada semestre: ${fmt(r.comInicialSem)} / Meta: ${fmt(bono.meta.comAcum)} → Falta: ${fmt(bono.fCom)}
Puntos acumulados semestre: ${fmtN(r.puntosSem)} / Meta: ${bono.meta.ptosAcum} → Faltan: ${fmtN(bono.fPtos)} puntos
Bono en juego: ${fmt(bono.meta.premMax)} base + excedente al 35%
Estado: ${bono.cumple?'✅ YA CALIFICA — maximizar excedente':'⚠️ AÚN NO CALIFICA'}
Comisiones iniciales este mes: ${fmt(r.comInicialMes)} | Renovación: ${fmt(r.comRenovMes)}
Pólizas emitidas mes: ${r.detallesMes.length} | Puntos mes: ${fmtN(r.puntosMes)}`;
        } else {
            ctx=`Cuaderno: Nuevo Profesional
Prima meta semestral: ${fmt(r.primaMetaSem)} → Grupo ${bono.grupo||'sin grupo'}
LIMRA: ${bono.limra}% | IGC: ${bono.igc}%
Bono Inicial Vida: ${fmt(bono.montoBI)} al ${(bono.pct*100).toFixed(1)}%
Bono GMM trimestral: ${fmt(bono.montoGMM)} ${bono.grupoGMM?`Grupo ${bono.grupoGMM.g}`:'sin grupo'}
Total bonos: ${fmt(bono.total)}`;
        }

        const prompt=`Eres coach comercial de élite especializado en Seguros Monterrey New York Life (SMNYL).

Datos reales del asesor — ${mesNom}:
${ctx}

Genera exactamente 4 estrategias concretas ejecutables ESTA SEMANA. Cada una:
- TÍTULO CORTO EN MAYÚSCULAS
- 2-3 líneas de acción específica con productos SMNYL reales (Star Temporal, Alfa Medical, Mío, Orvi 99, Imagina Ser, etc.)
- Impacto estimado en comisiones o puntos

Sin relleno. Sin introducciones. Solo las 4 estrategias numeradas.`;
        callGemini(prompt, 'out-tips');
    });

    // Modo Dev
    document.getElementById('btn-dev-reset')?.addEventListener('click', async () => {
        const ok = await showConfirm('Esto borrará tu perfil financiero para reconfigurarlo. ¿Continuar?','Resetear Perfil','Resetear',true);
        if(!ok) return;
        try {
            ...
            window.navigateTo('comisiones');
        } catch(e){ showToast('Error al resetear perfil.','danger'); }
    });
}
