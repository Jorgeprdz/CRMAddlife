// /modules/comisiones.js - Motor Financiero Real con Desglose de Auditoría Semántica
...
const ActuarialEngine = {
    calculatePortfolio(cartera, perfil) {
        ...
        const results = {
            ...
            brechaBonoVidas: 0,
            calificaBono: false,
            historicoMeses: [0, 0, 0, 0, 0, 0],
            auditoriaPolizasMes: [] // Almacenará el desglose para el cuadro de transparencia
        };
        ...
        cartera.forEach(p => {
            ...
            const tasaComisionAplicada = esPrimerAño ? (tabla.nn * factorDesarrollo) : tabla.ry;
            const comisionLiquida = primaNeta * factorFrecuencia * tasaComisionAplicada;
            ...
            // Mes en Curso: Acumuladores y Auditoría
            if (fechaCobro.getMonth() === hoy.getMonth() && fechaCobro.getFullYear() === hoy.getFullYear()) {
                ...
                // Inyección al pool de auditoría transparente
                results.auditoriaPolizasMes.push({
                    cliente: p.cliente || 'Sin Nombre',
                    plan: planLimpio,
                    formaPago: p.formaPago || 'Anual',
                    primaAnual: primaNeta,
                    tipo: esPrimerAño ? 'Inicial' : 'Renovación',
                    porcentaje: (tasaComisionAplicada * 100).toFixed(0) + '%',
                    montoMuestra: comisionLiquida
                });
            }
        });
        ...
        if (esquemaActual === 'Desarrollo') {
            ...
            results.brechaBonoMonto = Math.max(0, 30000 - results.mesIniciales);
        }
...
const FinancialUI = {
    ...
    renderConfigForm() {
        return `
            <div id="fin-dashboard-container" class="glass-widget" style="margin:20px 10px; padding:16px;">
                <h2 style="font-size:18px; margin-bottom:16px; font-weight:700;">⚙️ Configuración Inicial</h2>
                <p style="font-size:12px; color:var(--text-secondary); margin-bottom:16px;">El sistema detectará el Cuaderno de Concurso basándose en tu fecha de alta contractual.</p>
                <div style="display:flex; flex-direction:column; gap:16px;">
                    <div>
                        <label style="font-size:11px; font-weight:600; color:var(--text-secondary);">Fecha de Conexión</label>
    ...
    renderChart(datos) {
        const maxVal = Math.max(...datos, 1000);
        const meses = ['M-5', 'M-4', 'M-3', 'M-2', 'M-1', 'Mes Actual'];
        const barras = datos.map((val, i) => {
            const altura = (val / maxVal) * 100;
            return `
                <div style="display:flex; flex-direction:column; align-items:center; flex:1; gap:4px;">
                    <div style="width:100%; height:90px; background:rgba(150,150,150,0.08); border-radius:8px; display:flex; align-items:flex-end;">
                        <div style="width:100%; height:${altura}%; background:var(--accent); border-radius:6px; transition:height 0.5s ease;"></div>
                    </div>
                    <span style="font-size:9px; color:var(--text-secondary); font-weight:700;">${meses[i]}</span>
                </div>
            `;
        }).join('');
        return `<div style="display:flex; gap:8px; width:100%; padding-top:10px;">${barras}</div>`;
    },
    ...
    hydrateDashboard(data) {
        ...
        let bonoUI = isDesarrollo 
            ? (data.calificaBono 
                ? `<div style="display:flex; justify-content:space-between; align-items:center; margin-top:8px;"><span style="font-size:24px; font-weight:800;">${this.fmt(data.bonoMesCalculado)}</span><span class="status-badge" style="background:rgba(52,199,89,0.1); color:#34C759;">✅ Logrado</span></div>` 
                : `<div style="display:flex; justify-content:space-between; align-items:center; margin-top:8px;"><span style="font-size:24px; font-weight:800;">$0.00</span><span class="status-badge" style="background:rgba(255,149,0,0.1); color:#FF9500;">${data.mesPuntosConcurso} Vidas</span></div>
            ...
            : (data.calificaBono 
                ? `<div style="display:flex; justify-content:space-between; align-items:center; margin-top:8px;"><span style="font-size:24px; font-weight:800;">${this.fmt(data.bonoMesCalculado)}</span><span class="status-badge" style="background:rgba(52,199,89,0.1); color:#34C759;">✅ Escala Alcanzada</span></div>`
                : `<div style="display:flex; justify-content:space-between; align-items:center; margin-top:8px;"><span style="font-size:24px; font-weight:800;">$0.00</span><span class="status-badge" style="background:rgba(255,149,0,0.1); color:#FF9500;">Fuera de Rango</span></div>
                   <p style="font-size:12px; color:var(--text-secondary); margin-top:8px;">Faltan <strong style="color:var(--danger);">${this.fmt(data.brechaBonoMonto)}</strong> de producción inicial para el primer escalón.</p>`);

        // Render del cuadro de auditoría (Transparencia Actuarial)
        let tablaAuditoriaHTML = `
            <div style="text-align:center; padding:16px; color:var(--text-secondary); font-size:13px;">
                No hay transacciones registradas o cobradas en el mes en curso.
            </div>`;
            
        if (data.auditoriaPolizasMes.length > 0) {
            tablaAuditoriaHTML = `
                <div style="overflow-x:auto; width:100%; -webkit-overflow-scrolling:touch;">
                    <table style="width:100%; border-collapse:collapse; text-align:left; font-size:12px;">
                        <thead>
                            <tr style="border-bottom:2px solid var(--separator); color:var(--text-secondary);">
                                <th style="padding:10px 6px; font-weight:700;">Cliente / Plan</th>
                                <th style="padding:10px 6px; font-weight:700; text-align:center;">Frecuencia</th>
                                <th style="padding:10px 6px; font-weight:700; text-align:right;">PNA</th>
                                <th style="padding:10px 6px; font-weight:700; text-align:center;">%</th>
                                <th style="padding:10px 6px; font-weight:700; text-align:right;">Líquido</th>
                            </tr>
                        </thead>
                        <tbody>
                            ${data.auditoriaPolizasMes.map(row => `
                                <tr style="border-bottom:1px solid rgba(150,150,150,0.1); font-weight:500;">
                                    <td style="padding:12px 6px;">
                                        <div style="font-weight:700; color:var(--text-primary);">${row.cliente}</div>
                                        <div style="font-size:10px; color:var(--text-secondary); margin-top:2px;">${row.plan} (${row.tipo})</div>
                                    </td>
                                    <td style="padding:12px 6px; text-align:center; color:var(--text-secondary);">${row.formaPago}</td>
                                    <td style="padding:12px 6px; text-align:right;">${this.fmt(row.primaAnual)}</td>
                                    <td style="padding:12px 6px; text-align:center;"><span style="background:rgba(0,122,255,0.08); color:#007AFF; padding:2px 6px; border-radius:6px; font-weight:700; font-size:10px;">${row.porcentaje}</span></td>
                                    <td style="padding:12px 6px; text-align:right; font-weight:700; color:var(--success);">${this.fmt(row.montoMuestra)}</td>
                                </tr>
                            `).join('')}
                        </tbody>
                    </table>
        ...
        return `
            <div id="fin-dashboard-container" style="display:flex; flex-direction:column; gap:16px; padding-bottom:24px; width:100%;">
                
                <div class="glass-widget" style="background:var(--text-primary); color:var(--surface); border:none; text-align:center; padding:24px;">
                    <span style="font-size:12px; opacity:0.8; font-weight:600; text-transform:uppercase; letter-spacing:0.5px;">Ingreso Estimado Mes Actual</span>
                ...
                <div style="display:grid; grid-template-columns:1fr 1fr; gap:10px;">
                    <div class="glass-widget" style="padding:16px;">
                        <span style="font-size:11px; opacity:0.6; font-weight:700;">INICIALES (MES)</span>
                        <div style="font-size:18px; font-weight:800; margin-top:4px;">${this.fmt(data.mesIniciales)}</div>
                    </div>
                    <div class="glass-widget" style="padding:16px;">
                        <span style="font-size:11px; opacity:0.6; font-weight:700;">RENOVACIÓN (MES)</span>
                        <div style="font-size:18px; font-weight:800; margin-top:4px;">${this.fmt(data.mesRenovacion)}</div>
                    </div>
                </div>
                ...
                <div class="glass-widget" style="background:rgba(0,122,255,0.04); border-left:4px solid var(--accent); padding:16px;">
                    ...
                    <div id="ai-finance-tip" style="font-size:13px; margin-top:8px; color:var(--text-secondary); line-height:1.4;">Haz clic en Analizar para diagnóstico.</div>
                </div>
                ...
                <h3 style="font-size:14px; font-weight:700; color:var(--text-secondary); margin: 8px 0 0 4px;">🔍 Desglose Actuarial del Mes</h3>
                <div class="glass-widget" style="padding:16px;">
                    ${tablaAuditoriaHTML}
                </div>
                ...
                <div class="glass-widget" style="padding:16px;">
                    <div style="display:flex; justify-content:space-between; align-items:center; padding-bottom:12px; border-bottom:1px solid rgba(150,150,150,0.1);">
                        ...
                        <span style="font-size:16px; font-weight:800; color:var(--text-primary);">${this.fmt(data.anualTotal)}</span>
                    </div>
                    <div style="display:flex; justify-content:space-between; align-items:center; padding-top:12px;">
                        ...
                        <span style="font-size:16px; font-weight:800; color:#34C759;">${this.fmt(data.anualInicialesConvencion)}</span>
                    </div>
                </div>
...
export async function bindComisionesEvents() {
    ...
    try {
        ...
        document.getElementById('btn-ai-finance')?.addEventListener('click', async () => {
            ...
            out.innerHTML = '<span class="spinner-mini">⚙️</span> Analizando transacciones...';
            
            const prompt = `Actúas como CFO personal del asesor. Esquema contractual activo: ${resultados.esquema}.
            Comisiones calculadas este mes: $${resultados.mesIniciales} iniciales. 
            Brecha de bonificaciones: faltan $${resultados.brechaBonoMonto} y ${resultados.brechaBonoVidas} pólizas.
            REGLA: Dame 1 recomendación de enfoque comercial (máx 2 líneas) sin saludos.`;
            
            await callGemini(prompt, 'ai-finance-tip');
        });

        document.getElementById('btn-dev-reset')?.addEventListener('click', async () => {
            const seguro = await showConfirm('Esto reseteará tu perfil de comisiones para recalibrar los cuadernos. ¿Continuar?', 'Modo Dev', 'Confirmar', true);
            if (seguro) {
